...
import TabItemPrefer from '../TabItemPrefer'
...
class MainItemsDisplay extends Component {
  state = {
    activeTabId: tabsList[0].tabId,
    imageSelected: imagesList[0],
    score: 0,
    setTimeDisplay: 60,
    clickedWrong: false,
  }
  ...
  tick = () =>
    this.setState(prevState => ({
      setTimeDisplay: prevState.setTimeDisplay - 1,
    }))

  selectedThisTab = testId => {
    this.setState({activeTabId: testId})
  }

  callToClear = () => {
    this.setState({clickedWrong: true})
    this.clearTimerInterval()
  }
  ...
  displayTropy = score => (
    <div className="Wincard-Display-COn">
      <img
  ...
  FF = (imageSelected, finalFilter, activeTabId) => (
    <>
      <div className="img-Displayed-container">
        <img
          src={imageSelected.imageUrl}
          alt={imageSelected.category}
          className="img-Displayed"
        />
      </div>

      <ul className="Ul-list-Main-COn">
        {tabsList.map(itm => (
          <TabItemPrefer
            key={itm.tabId}
            selectedThisTab={this.selectedThisTab}
            everyItem={itm}
            isActive={activeTabId === itm.tabId}
          />
        ))}
      </ul>

      {/* ******************************************************************** */}
      <div className="container-tumbnail-con">
        <ul className="tumbnail-display-Ul">
          {finalFilter.map(itm => (
            <ImageItems
              key={itm.thumbnailUrl}
              thiImgSelected={this.thiImgSelected}
              itemsProps={itm}
            />
          ))}
        </ul>
  ...
  thiImgSelected = id => {
    ...
    if (id === imageSelected.id) {
      this.setState({imageSelected: filterId})
  ...
  render() {
    ...
    const finalFilter = imagesList.filter(
      itm => itm.category.toLowerCase() === activeTabId.toLowerCase(),
    )
    const TT = clickedWrong
      ? this.displayTropy(score)
      : this.FF(imageSelected, finalFilter, activeTabId)

    if (clickedWrong) {
      this.clearTimerInterval()
    } else if (setTimeDisplay === 0) {
      this.callToClear()
    }
      ...
      <div className="main-Container">
        <nav className="Nav-bar-Con">
          <img
            ...
            alt="website logo"
            className="logo-Con-img"
          />

          <div className="Nav-bar-Score-Time-Con">
            <p className="nav-score-para">Score:{score}</p>
            <div className="nav-time-con">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
                alt="timer"
                className="nav-time-img"
              />
              <p className="nav-sec-display">{setTimeDisplay}:sec</p>
            </div>
          </div>
        </nav>

        {/* ************************************************* */}
        <div className="Sec-Main-Con">{TT}</div>
      </div>
    )
  }
}
